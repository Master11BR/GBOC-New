...
def ensure_agent_exists(conn, agent_id: str, data: Optional[Dict] = None):
    ...
    cur.execute('''
        ...
        ON CONFLICT (agent_id) DO UPDATE SET
            ...
            status = 'online'
    ''', (agent_id, hostname, ip_address, os_info, agent_version, tools))
...
async def stats():
    ...
    try:
        ...
        agt = cur.fetchone()
        # Usar agent_task_executions (dados reais) + backup_reports como fallback
        cur.execute("""
            WITH combined AS (
...
async def recent_backups(limit: int = 20):
    conn = None
    try:
        conn = get_db(); cur = conn.cursor(cursor_factory=RealDictCursor)
        # Priorizar execuções sincronizadas (agent_task_executions), pois trazem task_id
        # e permitem exibir identificador + nome da tarefa de forma consistente.
        cur.execute("""
            SELECT
                te.status,
                te.agent_id,
        ...
        # Fallback para instalações legadas sem agent_task_executions
        cur.execute("""
...
try:
    ...
    logger.warning(f"Falha ao carregar módulos no Servidor: {_e}")

# --- MULTI-TENANT MSP API ---
_msp_organizations_store = [
    {"id": "org-master", "name": "Master Enterprise MSP", "plan": "Enterprise 10x", "max_agents": 250},
    {"id": "org-filial-01", "name": "Filial São Paulo (Financeiro)", "plan": "Pro Managed", "max_agents": 50},
    {"id": "org-filial-02", "name": "Filial Rio de Janeiro (Operações)", "plan": "Standard", "max_agents": 25}
]

@app.get("/api/v1/tenant/organizations")
async def get_tenant_organizations():
    """Retorna lista de Organizações Multi-Tenant MSP."""
    return {"status": "success", "organizations": _msp_organizations_store}

@app.post("/api/v1/tenant/organizations")
async def create_tenant_organization(request: Request):
    """Cria nova Organização MSP."""
    try:
        body = await request.json()
        org_id = f"org-{len(_msp_organizations_store) + 1:02d}"
        new_org = {
            "id": org_id,
            "name": body.get("name", "Nova Organização MSP"),
            "plan": body.get("plan", "Enterprise 10x"),
            "max_agents": int(body.get("max_agents", 50))
        }
        _msp_organizations_store.append(new_org)
        return {"status": "success", "message": "Organização MSP criada com sucesso", "organization": new_org}
    except Exception as e:
        raise HTTPException(500, detail=str(e))

@app.put("/api/v1/tenant/organizations/{org_id}")
async def update_tenant_organization(org_id: str, request: Request):
    """Atualiza dados e cotas de uma Organização MSP."""
    try:
        body = await request.json()
        for org in _msp_organizations_store:
            if org["id"] == org_id:
                if "name" in body: org["name"] = body["name"]
                if "plan" in body: org["plan"] = body["plan"]
                if "max_agents" in body: org["max_agents"] = int(body["max_agents"])
                return {"status": "success", "message": "Organização atualizada", "organization": org}
        raise HTTPException(404, detail="Organização não encontrada")
    except Exception as e:
        raise HTTPException(500, detail=str(e))

@app.delete("/api/v1/tenant/organizations/{org_id}")
async def delete_tenant_organization(org_id: str):
    """Remove uma Organização MSP."""
    global _msp_organizations_store
    _msp_organizations_store = [o for o in _msp_organizations_store if o["id"] != org_id]
    return {"status": "success", "message": "Organização MSP removida."}
...
async def get_server_users():
    """Retorna a lista de usuários do servidor."""
    return {"status": "success", "users": _server_users_store}

@app.post("/api/v1/users")
async def create_server_user(request: Request):
    """Cria um novo usuário no servidor."""
    try:
        body = await request.json()
        new_user = {
            "id": len(_server_users_store) + 1,
            "username": body.get("username", f"user_{len(_server_users_store)+1}"),
            "display_name": body.get("display_name") or body.get("username"),
            "role": body.get("role", "operator"),
            "status": "active"
        }
        _server_users_store.append(new_user)
        return {"status": "success", "message": "Usuário criado", "user": new_user}
    except Exception as e:
        raise HTTPException(500, detail=str(e))

@app.put("/api/v1/users/{user_id}")
async def update_server_user(user_id: int, request: Request):
    """Atualiza nome de exibição, perfil ou status do usuário."""
    try:
        body = await request.json()
        for u in _server_users_store:
            if u["id"] == user_id:
                if "display_name" in body: u["display_name"] = body["display_name"]
                if "role" in body: u["role"] = body["role"]
                if "status" in body: u["status"] = body["status"]
                return {"status": "success", "message": "Usuário atualizado", "user": u}
        raise HTTPException(404, detail="Usuário não encontrado")
    except Exception as e:
        raise HTTPException(500, detail=str(e))

@app.delete("/api/v1/users/{user_id}")
async def delete_server_user(user_id: int):
    """Exclui um usuário do servidor."""
    global _server_users_store
    _server_users_store = [u for u in _server_users_store if u["id"] != user_id]
