...
async def server_ransomware_overview():
    """Aggregated ransomware status across all agents"""
    conn = None
    try:
        conn = get_db(); cur = conn.cursor(cursor_factory=RealDictCursor)
        cur.execute("""
            SELECT a.agent_id, a.hostname, a.current_status
            FROM (
                SELECT *, CASE WHEN last_heartbeat > (LOCALTIMESTAMP - INTERVAL '60 minutes') THEN 'online' ELSE 'offline' END as current_status
                FROM agents
            ) a ORDER BY a.hostname
        """)
        agents = cur.fetchall()
        return {
            "protected_agents": len([a for a in agents if a.get('current_status') == 'online']),
            "total_canaries": 0,
            "total_compromised": 0,
            "scans_7d": 0,
            "agents": [{
                "hostname": a['hostname'],
                "is_protected": a.get('current_status') == 'online',
                "canaries": 0,
                "compromised": 0,
                "last_scan": None,
                "threat_level": "none"
        ...
        logger.error(f"Erro ransomware overview: {e}")
