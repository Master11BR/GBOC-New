...
# Module: Hardware, Disks & S.M.A.R.T. Telemetry Router (Agent)
...
try:
    ...
    def build_v2_response(data, execution_time_ms=None):
        return {"success": True, "api_version": "v2", "data": data, "execution_time_ms": execution_time_ms}
...
logger = logging.getLogger("gboc_agent_hardware_api")
router = APIRouter(tags=["Hardware & SMART"])

_AGENT_AMBIENT_CACHE = {
    "data": None,
    "last_fetch": 0
}

def get_agent_ambient_telemetry() -> Dict[str, Any]:
    """Coleta a temperatura real do ambiente externo da cidade onde o agente está rodando."""
    global _AGENT_AMBIENT_CACHE
    now = time.time()
    if _AGENT_AMBIENT_CACHE["data"] and (now - _AGENT_AMBIENT_CACHE["last_fetch"] < 600):
        res = dict(_AGENT_AMBIENT_CACHE["data"])
        res["cached"] = True
        return res

    try:
        req_loc = urllib.request.Request("http://ip-api.com/json", headers={"User-Agent": "GBOC-Agent-Hardware-Telemetry/1.0"})
        with urllib.request.urlopen(req_loc, timeout=3) as resp_loc:
            ...
            if lat is not None and lon is not None:
                ...
                req_w = urllib.request.Request(url_weather, headers={"User-Agent": "GBOC-Agent-Hardware-Telemetry/1.0"})
                with urllib.request.urlopen(req_w, timeout=3) as resp_w:
                    ...
                    _AGENT_AMBIENT_CACHE["data"] = result
                    _AGENT_AMBIENT_CACHE["last_fetch"] = now
                    return result
    except Exception as e:
        logger.warning(f"Não foi possível obter clima ambiente externo no Agente: {e}")

    if _AGENT_AMBIENT_CACHE["data"]:
        res = dict(_AGENT_AMBIENT_CACHE["data"])
        res["cached"] = True
        return res
...
def get_agent_hardware_telemetry() -> Dict[str, Any]:
    """
    Coleta telemetria 100% REAL do hardware do host do Agente:
    ...
    - Partições de Armazenamento: Ponto de montagem, Sistema de arquivos, Usado, Livre, Total, Percentual.
    - Memória RAM: Total, Usado, Livre, Percentual.
    - Ambiente Externo & Comparativo Térmico.
    """
    ...
    if platform.system() == "Windows":
        try:
            ...
            logger.warning(f"Erro ao consultar Get-PhysicalDisk no Agente: {ex}")
    ...
    # 5. Ambient Weather Telemetry (Cidade onde o agente roda)
    ambient_data = get_agent_ambient_telemetry()
...
async def get_agent_hardware_v1(request: Request):
    """Endpoint legado/v1 para telemetria de hardware, S.M.A.R.T. e ambiente no Agente."""
    return get_agent_hardware_telemetry()

@router.get("/api/v2/system/hardware")
async def get_agent_hardware_v2(request: Request):
    """Endpoint moderno v2 com envelope para telemetria de hardware, S.M.A.R.T. e ambiente no Agente."""
    t0 = time.perf_counter()
    data = get_agent_hardware_telemetry()
