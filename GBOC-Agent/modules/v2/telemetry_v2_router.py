...
# Module: Agent Telemetry & System Router (API v2)
# ==============================================================================
...
router = APIRouter(prefix="/system", tags=["Agent System v2"])

_AGENT_START_TIME = time.time()

def _get_agent_telemetry():
    """Coleta métricas 100% reais do agente local e hardware."""
...
async def get_agent_version_v2():
    ...
    try:
        ...
        v_info = {"raw_version": "14.1.0", "semver": "14.1.0"}
    
    elapsed = (time.perf_counter() - t0) * 1000
    return build_v2_response(data=v_info, execution_time_ms=elapsed)

@router.get("/info")
async def get_agent_info_v2():
    """Retorna a telemetria completa de hardware e ambiente operacional do agente."""
    t0 = time.perf_counter()
    telemetry = _get_agent_telemetry()
    
    try:
        from version_control import get_version_info
        telemetry["version"] = get_version_info()
    except Exception:
        telemetry["version"] = {"raw_version": "14.1.0"}
        
    elapsed = (time.perf_counter() - t0) * 1000
    return build_v2_response(data=telemetry, execution_time_ms=elapsed)

@router.get("/stream")
async def stream_agent_telemetry_v2(interval: int = Query(2, ge=1, le=60)):
    """Server-Sent Events (SSE) stream para telemetria em tempo real do agente."""
